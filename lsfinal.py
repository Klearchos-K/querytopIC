...
class Graph_l():
    ...
    def local_search(self, k, r, maxk):
        extend = False
        if r in self.t:
            t = self.t[r]
            if t == 'full':
        ...
        self.con_tree1111(cvs[:num], r, maxk)
        if i == len(self.G):
    ...
    def query(self, k, r, maxk):
        if r not in self.IT:
            print("no icp tree found!")
            self.local_search(k, r, maxk)
